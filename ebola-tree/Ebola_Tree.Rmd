...
```{r Formatting the full ebola tree data and calculating node probabilities}
#Ebola Data
...
```{r Visualizing the tree with node probabilities}
ebola2 <- ebola
...
```{r Visualizing outliers by Z-scores in the full tree}
...
```{r Visualizing Outliers on the tree}
#With tip labels
...
```{r Node probabilities plotted against tips within the clades}
...
```{r Coloring the full ebola tree by the clades in the scatterplot above}
x <-zs$i
